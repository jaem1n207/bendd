...
인터랙션 디자인 기술에 대해 공부할 때 우연히 [rauno](https://github.com/raunofreiberg) 라는 분을 알게 되었고 이 분의 [rauno-projects](https://rauno.me/projects)에서 각 프로젝트를 렌더링할 때 나오는 애니메이션이 저에게 너무나 매력적인 시각적 효과로 다가왔습니다.
...
<video width="100%" controls preload="metadata" muted playsInline>
  <source
    src="/videos/craft/implement-rauno-style-text-animation/rauno-projects-animation.mp4"
    type="video/mp4"
  />
...
<video width="100%" controls preload="metadata" muted playsInline>
  <source
    src="/videos/craft/implement-rauno-style-text-animation/animated-text.mp4"
    type="video/mp4"
  />
  Your browser does not support the video tag.
</video>
...
<video width="100%" controls preload="metadata" muted playsInline>
  <source
    src="/videos/craft/implement-rauno-style-text-animation/new-animated-text.mp4"
    type="video/mp4"
  />
  Your browser does not support the video tag.
</video>
...
```tsx
// 1.
const calculateEvenSum = arr =>
  arr.filter(num => !(num & 1)).reduce((acc, val) => acc + val, 0);
