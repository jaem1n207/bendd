...
<Callout type="info" emoji="✨">
**zustand와 Context API** - 많은 분들이 zustand와 Context API를 '전역 상태 관리'라는 같은 범주로 비교하시는데, 사실 두 도구의 본질은 달라요. Context API는 의존성 주입을 위한 도구이고, zustand는 진짜 상태 관리를 위한 도구예요. 성능 차이뿐만 아니라 각 도구를 언제, 어떻게 사용해야 하는지 궁금하다면 [Zustand vs Context API: 올바른 선택과 사용법](../performance/zustand-vs-context-performance)을 확인해보세요!
</Callout>
...
<Callout type="info" emoji="📚">
- **성능 최적화**: [스크립트 로딩 최적화 전략](../performance/script-loading-optimization)에서 조건부 스크립트 로딩 방법을 알아보세요
- **상태 관리**: [Zustand vs Context API: 올바른 선택과 사용법](../performance/zustand-vs-context-performance)에서 더 자세한 가이드를 확인하세요
</Callout>
