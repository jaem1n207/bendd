...
bendd.me는 Article/Craft 상세 본문에 별도 손글씨 웹폰트를 적용하지 않는다. Gaegu를 실제 페이지에 적용해본 결과, 사이트의 개인적인 분위기는 강해졌지만 긴 한글 본문에서 글자 형태가 고르지 않고 문장 단위 스캔 비용이 커졌다. 본문 폰트는 장식이 아니라 읽기 인프라이므로, 최종 선택은 기존 `Inter` 기반 sans 폰트를 유지하는 것이다.

현재 정책은 다음과 같다.

- 전역 sans 폰트는 root layout의 `--font-sans`를 사용한다.
- 전역 mono 폰트는 root layout의 `--font-mono`를 사용한다.
- Article/Craft 상세 본문은 별도 content-only 웹폰트를 로드하지 않는다.
...
**Response:** Article/Craft 상세 본문은 기존 `--font-sans`를 유지한다. 본문 전용 custom font는 로드하지 않는다.

**Why:** bendd.me의 상세 페이지는 긴 한국어 technical writing이 핵심이다. 감성보다 읽기 지속 시간이 더 중요하다.
...
**Response:** Article/Craft 상세 본문에 새 webfont를 추가하지 않는다. 전역 font는 Next.js `next/font`의 self-hosting과 `display: 'swap'` 정책을 사용한다.
...
**Response:** 본문 전용 추가 webfont를 제거해 본문에서 새 swap event가 발생하지 않게 한다. 기존 root font만 사용하고, article body에는 stable line-height를 둔다.

**Why:** custom content font를 추가하면 Korean glyph subset이 도착하는 타이밍마다 본문이 다시 그려질 수 있다. 블로그 상세 페이지에서는 안정적인 reading position이 더 중요하다.
...
**Response:** content-only font를 제거하고 기존 root font contract를 유지한다. Article/Craft body는 `line-height: 1.9`로 여유를 확보한다.

**Why:** metrics override로 보정할 수도 있지만, 장식적 본문 폰트를 위해 별도 metrics tuning을 운영하는 것은 복잡도 대비 가치가 낮다.
...
**Response:** 상세 본문 전용 한글 custom font를 로드하지 않는다. 기존 전역 font만 사용한다.

**Why:** Article/Craft 상세 페이지는 본문이 길고 이미지, MDX component, code block도 함께 렌더링된다. 여기에 큰 font payload를 추가하면 LCP와 bandwidth에 직접 영향을 준다.
...
**Response:** 본문 전용 추가 font를 없애 preload 판단 자체를 제거한다. 전역 font는 root layout에서만 관리한다.

**Why:** page-specific decorative font를 preload하면 critical path 비용이 증가한다. preload가 필요할 정도로 중요한 font라면, 그 font는 장식이 아니라 product typography로 검토되어야 한다.
...
**Response:** `next/font` self-hosting만 사용한다. E2E에서는 font request가 현재 origin에서만 발생하는지 확인한다.
...
**Response:** 본문은 기존 sans stack을 사용한다. emoji는 browser/platform fallback에 맡기고, code와 UI control은 별도 font family로 격리한다.

**Why:** 기술 글은 한글, 영어, 숫자, code token, emoji, 링크가 섞인다. 특정 손글씨 폰트 하나가 이 모든 glyph를 안정적으로 처리하기 어렵다.
...
**Response:** Article body safeguard에 `font-synthesis-weight: none`을 둔다. 본문에 별도 custom font를 추가하지 않아 weight coverage 운영 범위를 줄인다.
...
**Response:** 본문 custom font를 제거하고 system-friendly sans stack을 유지한다. E2E 접근성 테스트를 계속 실행한다.
...
- `MdxLayout` does not import a content-specific font from `next/font/google`.
- `article[data-content-font="system"]` marks the current contract.
...
- Article detail body uses the existing sans font and does not contain Gaegu.
- Craft detail body uses the same font contract.
- Font requests stay on the same origin.
- No Gaegu `@font-face` rule is present in runtime stylesheets.
...
Do not add a new Article/Craft content font unless all of the following are true:
...
- Payload, preload, `font-display`, fallback metrics, and low-end device behavior are documented.
