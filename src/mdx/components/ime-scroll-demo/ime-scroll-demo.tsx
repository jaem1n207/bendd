...
function ImeScrollDemo({ initialText = '' }: ImeScrollDemoProps) {
  ...
  const imeScrollHandlers = useImeSafeScroll(
    fixedInputRef as React.RefObject<HTMLInputElement>,
    fixedValue
  );
    ...
    <div className="bd:my-6 bd:space-y-6 bd:rounded-lg bd:border-2 bd:p-6">
      <div className="bd:space-y-2 bd:text-center">
        ...
        <div className="bd:text-xs bd:text-muted-foreground">
          현재 브라우저:{' '}
          <ClientGate>
            <span className="bd:rounded-sm bd:bg-muted bd:px-2 bd:py-1 bd:text-muted-foreground">
              {isChromeBrowser()
                ? 'Chrome (버그 재현 가능)'
                : '다른 브라우저 (정상 동작)'}
            </span>
      ...
      <div className="bd:space-y-4">
        ...
        <div className="bd:space-y-3">
          ...
          <div className="bd:flex bd:flex-col bd:gap-2">
            <Input
              ...
              className="bd:!w-[120px] bd:border-destructive bd:text-sm bd:focus:ring-destructive"
            />
            <button
              onClick={() => {
              ...
              className="bd:self-start bd:rounded-sm bd:bg-destructive bd:px-2 bd:py-1 bd:text-xs bd:text-destructive-foreground bd:transition-colors bd:hover:bg-destructive/90"
            >
              긴 텍스트로 채우기
            </button>
          </div>
        ...
        <div className="bd:space-y-3">
          <div className="bd:flex bd:items-center bd:gap-2">
            <span className="bd:text-green-700 bd:dark:text-green-400">✅</span>
            <h5 className="bd:text-sm bd:font-semibold bd:text-green-700 bd:dark:text-green-400">
          ...
          <div className="bd:flex bd:flex-col bd:gap-2">
            <Input
              ...
              placeholder="긴 한글 텍스트를 입력해보세요..."
              className="bd:!w-[120px] bd:border-green-500 bd:text-sm bd:focus:ring-green-600"
            />
            <button
              onClick={() => {
              ...
              className="bd:self-start bd:rounded-sm bd:bg-green-600 bd:px-2 bd:py-1 bd:text-xs bd:text-white bd:transition-colors bd:hover:bg-green-700"
            >
              긴 텍스트로 채우기
            </button>
          </div>
